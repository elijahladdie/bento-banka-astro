...
import { loadPreference, savePreference } from '../utils/preferences.ts';
import { PreferenceName, PricingInterval, StorageScope } from '../types';

const pricingRootSelector = '[data-pricing-root]';

let currentPricingInterval: PricingInterval = PricingInterval.MONTH;
...
function getStoredPricingInterval() {
  return loadPreference(PreferenceName.PRICING_INTERVAL, null) as PricingInterval | null;
}

function persistPricingInterval(interval: PricingInterval) {
  savePreference(PreferenceName.PRICING_INTERVAL, interval, {
    storage: StorageScope.SESSION,
    cookie: true,
  });
}
...
function syncPricingLocaleLabels(locale: string) {
  ...
  root.dataset.pricingFreeLabel = translate(
    locale,
...
function syncPricingDisplay(locale: string) {
  ...
  document.querySelectorAll('[data-plan-id]').forEach((card) => {
    ...
    if (billingNode) {
      billingNode.textContent = getBillingLabel(
...
function initPricingToggle() {
  ...
  document.addEventListener('click', (event) => {
    ...
    const current =
      switchElement.dataset.currentInterval === PricingInterval.YEAR
        ? PricingInterval.YEAR
        : PricingInterval.MONTH;
    const next: PricingInterval =
      current === PricingInterval.MONTH ? PricingInterval.YEAR : PricingInterval.MONTH;
