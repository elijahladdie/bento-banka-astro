import { bootstrapLocale, getLocale, setLocale, subscribeLocale } from '../utils/i18n.ts';
...
function setLanguageSwitcherLabel(locale: string) {
  ...
  roots.forEach((root) => {
    const currentLabel = root.querySelector('[data-language-current]') as HTMLElement | null;

    const currentFlag = root.querySelector('[data-language-flag]') as HTMLElement | null;
    ...
    if (currentFlag) {
      currentFlag.className = flagMap[locale] || 'fi fi-gb';
    }
  });
}
...
function initDesktopLanguageSwitcher() {
  ...
  const toggle = desktopRoot.querySelector('[data-language-toggle]') as HTMLButtonElement | null;

  const panel = desktopRoot.querySelector('[data-language-panel]') as HTMLElement | null;
...
function getMobileLanguageRoot() {
  return document.querySelector('[data-language-dropdown="mobile"]') as HTMLElement | null;
}
...
function getMobileLanguageOptions(root: HTMLElement | null) {
  return Array.from(root?.querySelectorAll('[data-language-option]') ?? []) as HTMLElement[];
}
...
function initMobileLanguageSwitcher() {
  ...
  const open = () => {
    ...
    const chevron = root.querySelector('[data-language-chevron]') as HTMLElement | null;
